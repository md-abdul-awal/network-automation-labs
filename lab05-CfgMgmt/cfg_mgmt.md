...
The `proxytype` should now point to `netmiko`, while the `device_type` should be `juniper_junos` as documented in [https://docs.saltstack.com/en/master/ref/proxy/all/salt.proxy.netmiko_px.html](http://docs.saltproject.io/en/latest/ref/proxy/all/salt.proxy.netmiko_px.html), as this is what Netmiko expects.
